...
NEGATIVE_EDIT_ERR, EDIT_LIST_OBJ_ERR, UNIQUE_KEYS_ERR, NEW_ENTRY_ERR, UNDO_CONF_DICT
from utils.constants import IEEE802_DOT1CB_STREAM_IDENTIFICATION, IEEE802_DOT1CB_FRER

sid_undo_config = None
...
def validate_use_cases(model_name, data_from_board_all, use_case, yang_module_all, netconf_service, is_poison):
    ...
    try:
        ...
        if(len(use_cases.keys())):
            for key in sorted(use_cases.keys()):
                if "edit" in key:
                    ...
                    undo_config = add_new_entries_at_once(yang_module, working_config, use_cases[key], netconf_service)
            if is_poison:
                with pytest.raises(RPCError) as err:
                ...
                if model_name == IEEE802_DOT1CB_STREAM_IDENTIFICATION:
                    global sid_undo_config
                    sid_undo_config = undo_config
                #print(etree.tostring(sid_undo_config, encoding='unicode', pretty_print=True))
                if(model_name != IEEE802_DOT1CB_STREAM_IDENTIFICATION and UNDO_CONF_DICT[model_name] and None != undo_config):
                    netconf_service.edit_config_data('running', undo_config, 'merge')
                if (model_name == IEEE802_DOT1CB_FRER and None != sid_undo_config):
                    netconf_service.edit_config_data('running', sid_undo_config, 'merge')
        else:
        ...
        if(UNDO_CONF_DICT[model_name] and None != undo_config):
                        netconf_service.edit_config_data('running', undo_config, 'merge')
        expect(False, err)
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        ...
        #print(etree.tostring(backup_data, encoding='unicode', pretty_print=True))
...
def add_new_entries_at_once(yang_module, data_from_board, new_entries, netconf_service):
    undo_config = None
    '''Attempts to add all new entries defined in use cases at once, not objects one by one
       The variable data_from_board is modified
                                                     '''
    ''' in case of use-case test for frer or sid module'''
    tst_conf_root = etree.Element('config')
    tst_conf_root.set("xmlns", "urn:ietf:params:xml:ns:netconf:base:1.0")
    
    if "new_yang" in new_entries.keys():
        for new_yang_obj in new_entries['new_yang']['object']:
            if yang_module.name == "ieee802-dot1cb-frer":
                """" We need to add frer container  because it does not exists """
                new_el_cln = etree.Element('frer')
                new_el_cln.set("xmlns", "urn:ieee:std:802.1Q:yang:ieee802-dot1cb-frer")

                parse_entries(new_yang_obj['frer']['new_entries'], new_el_cln)
                tst_conf_root.append(new_el_cln) # add new container to root
            else:
                parse_entries(new_yang_obj['new_entries'], tst_conf_root)
  
        ''' apply config here '''
        #print(etree.tostring(tst_conf_root, encoding='unicode', pretty_print=True))
        netconf_service.edit_config_data('running', tst_conf_root, 'merge')

        ''' print data that is going to be send to the boar using edit-config merge operation '''
        #edited_config_data = netconf_service.get_config_data_all('running')
        #print(etree.tostring(edited_config_data, encoding='unicode', pretty_print=True))

        ''' create undo config '''
        for entry in tst_conf_root:
            entry.set("{urn:ietf:params:xml:ns:netconf:base:1.0}operation", "delete")
        
        undo_config = tst_conf_root
...
def add_multiple_config_element(new_entries, data_from_board, xpath_dict):
    ...
    if len(data_from_board.xpath(xpath_dict['xpath'], namespaces=xpath_dict['namespaces']))!=0:
        ...
        This part is used during test when we are adding new leafs in new container that does not exists in config on board '''
        prefix = ""
        xpath_components = xpath_dict['xpath'].split("/")
        missing_element = xpath_components[-1]
        if ":" in missing_element:
            prefix = missing_element.split(":")[0]
            container = missing_element.split(":")[1]
        else:
            container = missing_element
        xpath = xpath_dict['xpath'][:-(len(missing_element) + 1)]  # remove psfp:admin-control-list from xpath because we will add admin-control-list as child element
        xpath_dict['xpath'] = xpath
        root_element = get_container_element_bypath(data_from_board, xpath_dict)
        container_element = etree.Element(container)
        if prefix != "":
            namespace = xpath_dict['namespaces'][prefix]
            container_element.set("xmlns", namespace)
        root_element.append(container_element)
        root_element = container_element
    parse_entries(new_entries, root_element)
...
def parse_entries(new_entries, root_element):
    for new_entry in new_entries:
        ...
        if root_element is not None:
            ...
            if new_el_cln.tag == "stream-identity":
                new_el_cln.set("xmlns", "urn:ieee:std:802.1Q:yang:ieee802-dot1cb-stream-identification")
